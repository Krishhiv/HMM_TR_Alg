#!/usr/bin/env python3
"""
Download 1h OHLCV for the "Elite 10" assets from Coinbase Exchange.
Saves CSVs to data/coinbase_1h and writes metadata.json with start dates.
"""
...
GRAN_S = 3600
USER_AGENT = "elite10-1h-downloader/1.0"
...
def normalize_existing_btc(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path)
    if "Date" not in df.columns:
        raise ValueError("Expected a 'Date' column in existing BTC 1h CSV.")
    df["Date"] = pd.to_datetime(df["Date"], utc=True, errors="coerce")
    df = df.dropna(subset=["Date"]).set_index("Date").sort_index()
    cols = {c.lower(): c for c in df.columns}
    out = pd.DataFrame(index=df.index)
    out["Open"] = df[cols.get("open", "Open")]
    out["High"] = df[cols.get("high", "High")]
    out["Low"] = df[cols.get("low", "Low")]
    out["Close"] = df[cols.get("close", "Close")]
    out["Volume"] = df[cols.get("volume", "Volume")]
    out.index.name = "Date"
    return out
...
def main():
    ...
    parser.add_argument("--out-dir", default="data/coinbase_1h")
    ...
    btc_existing = Path("data_coinbase_btc/btc_1h.csv")

    for product in ELITE10:
        out_csv = out_dir / f"{product}_1h.csv"
        if out_csv.exists() and not args.overwrite:
            print(f"[skip] {product} (exists)")
            df = pd.read_csv(out_csv, parse_dates=["Date"])
            start_date = str(df["Date"].iloc[0])
            metadata[product] = {"start_date": start_date, "path": str(out_csv)}
            continue

        if product == "BTC-USD" and btc_existing.exists() and not args.overwrite:
            print("[reuse] BTC-USD from data_coinbase_btc/btc_1h.csv")
            df = normalize_existing_btc(btc_existing)
            df = forward_fill_missing(df)
            df.reset_index().to_csv(out_csv, index=False)
            metadata[product] = {"start_date": str(df.index[0]), "path": str(out_csv), "source": "data_coinbase_btc/btc_1h.csv"}
            continue
